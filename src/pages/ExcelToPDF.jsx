...
import { Upload, Download, Loader2, FileText, AlertCircle, FileSpreadsheet } from 'lucide-react'
...
function ExcelToPDF() {
	...
	useEffect(() => {
		setMeta({
			title: 'Excel to PDF - SheetlyPro',
			description: 'Convert Excel spreadsheets to PDF format. Fast and accurate XLSX to PDF conversion.',
			keywords: 'excel to pdf, xlsx to pdf, convert excel to pdf, excel converter',
			ogTitle: 'Excel to PDF Converter - SheetlyPro',
			ogDescription: 'Convert Excel spreadsheets to PDF files online',
		})
	}, [])
	...
	const handleFileSelect = (e) => {
		...
		if (selectedFile && selectedFile.name.toLowerCase().endsWith('.xlsx')) {
			...
			setError('Please select a valid Excel file (.xlsx only)')
		}
	}
	...
	const handleDrop = (e) => {
		...
		if (droppedFile && droppedFile.name.toLowerCase().endsWith('.xlsx')) {
			...
			setError('Please drop a valid Excel file (.xlsx only)')
		}
	}
	...
	const handleConvert = async () => {
		...
		try {
			...
			const response = await axios.post(
				'https://bank-statement-converter-backend-ofyc.onrender.com/api/pdf/excel-to-pdf/',
				formData,
			...
			if (response.data.data && response.data.data.id) {
				const downloadResponse = await axios.get(
					`https://bank-statement-converter-backend-ofyc.onrender.com/api/pdf/excel-to-pdf/${response.data.data.id}/download/`,
			...
			setError(err.response?.data?.error || 'Failed to convert Excel. Please try again.')
		} finally {
			setConverting(false)
		}
	}
			...
			<div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 md:py-20 lg:py-24">
				...
				<div className="max-w-4xl mx-auto">
					<div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl border border-gray-200 p-6 sm:p-8">
						...
						>
							<FileSpreadsheet className="h-12 w-12 sm:h-16 sm:w-16 mx-auto mb-4 text-purple-600" />
