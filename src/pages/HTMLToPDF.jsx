...
import { Upload, Download, Loader2, FileText, AlertCircle, Code } from 'lucide-react'
...
function HTMLToPDF() {
	...
	useEffect(() => {
		setMeta({
			title: 'HTML to PDF - SheetlyPro',
			description: 'Convert HTML web pages to PDF format. Fast and accurate HTML to PDF conversion.',
			keywords: 'html to pdf, convert html to pdf, html converter, web page to pdf',
			ogTitle: 'HTML to PDF Converter - SheetlyPro',
			ogDescription: 'Convert HTML files to PDF documents online',
		})
	}, [])
	...
	const handleFileSelect = (e) => {
		...
		if (selectedFile && selectedFile.name.toLowerCase().endsWith('.html')) {
			...
			setError('Please select a valid HTML file (.html only)')
		}
	}
	...
	const handleDrop = (e) => {
		...
		if (droppedFile && droppedFile.name.toLowerCase().endsWith('.html')) {
			setFile(droppedFile)
			setError(null)
		} else {
			setError('Please drop a valid HTML file (.html only)')
		}
	}
	...
	const handleConvert = async () => {
		...
		try {
			...
			const response = await axios.post(
				'https://bank-statement-converter-backend-ofyc.onrender.com/api/pdf/html-to-pdf/',
				formData,
			...
			if (response.data.data && response.data.data.id) {
				const downloadResponse = await axios.get(
					`https://bank-statement-converter-backend-ofyc.onrender.com/api/pdf/html-to-pdf/${response.data.data.id}/download/`,
			...
			setError(err.response?.data?.error || 'Failed to convert HTML. Please try again.')
		} finally {
			setConverting(false)
		}
	}
			...
			<div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 md:py-20 lg:py-24">
				...
				<div className="max-w-4xl mx-auto">
					<div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl border border-gray-200 p-6 sm:p-8">
						...
						>
							<Code className="h-12 w-12 sm:h-16 sm:w-16 mx-auto mb-4 text-purple-600" />
