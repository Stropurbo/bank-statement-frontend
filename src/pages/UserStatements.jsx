...
function UserStatements() {
	...
	const handleDelete = async (statementId) => {
		...
		try {
			...
			await AuthApiClient.delete(`statements/${statementId}/delete/`)
	...
	const handleBulkDelete = async () => {
		...
		for (const id of selectedItems) {
			try {
				await AuthApiClient.delete(`statements/${id}/delete/`)
			} catch (error) {
				console.error('Error deleting statement:', error)
			}
		}
					...
					<div className="flex items-center gap-4">
						...
						<div>
							...
							<p className="text-slate-600">
								View and manage your processed bank statements
							</p>
						</div>
					</div>
				...
				<div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
					{filteredAndSortedStatements.length === 0 ? (
						<div className="text-center py-12 text-slate-500">
							...
							<p className="text-sm">
								No processed statements match your search criteria
							</p>
						</div>
					) : (
								...
								<thead className="bg-slate-50 border-b border-slate-200">
									<tr>
										...
										<th
											...
											className="px-6 py-4 text-left text-xs font-medium text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100 select-none"
										>
