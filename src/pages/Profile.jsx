...
import { User, Crown, CheckCircle, AlertCircle, ArrowRight, Settings, CreditCard } from 'lucide-react'
		...
		try {
			...
			if (data.current_password && data.new_password) {
				try {
					...
					setSuccessMessage('Profile updated, but password change failed. Please try again.')
				}
			}
		} catch (error) {
								...
								<div className="bg-gradient-to-r from-purple-50 to-indigo-50 p-4 rounded-lg border border-purple-100">
									...
									{user?.usersubscription && (
										<div className="text-xs text-gray-600">
											<p>Remaining uploads: {user.usersubscription.remaining_uploads}</p>
											<p>Valid until: {new Date(user.usersubscription.end_date).toLocaleDateString()}</p>
										</div>
									)}
								</div>
								...
								<div className="bg-gray-50 p-4 rounded-lg">
									...
									<div className="flex items-center space-x-2">
										<CheckCircle className={`h-4 w-4 ${
											user?.usersubscription?.is_active ? 'text-green-600' : 'text-gray-400'
										}`} />
										<span className="text-sm text-gray-600">
											{user?.usersubscription?.is_active ? 'Subscription Active' : 'No Active Subscription'}
										</span>
									</div>
								</div>
								...
								<div className="pt-4 border-t border-gray-200">
									{!user?.usersubscription ? (
										<div className="text-center">
											<p className="text-xs text-gray-500 mb-3">Upgrade to unlock premium features!</p>
											<Link
												to="/pricing"
										...
										<div className="space-y-3">
											<p className="text-xs text-gray-500 text-center mb-3">Manage your subscription</p>
